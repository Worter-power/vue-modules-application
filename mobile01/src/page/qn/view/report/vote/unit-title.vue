    ...
    <div class="qn-report-container" v-loading="pageLoading">
        <div class="qn_statc_header sy-theme-border">
            <h3> 
                ...
                <el-button :disabled="!qnData.titleList.length" type="text" v-download="{ table: true, fileName: qnData.name+$store.state.lang.qn.b2 }">{{$store.state.lang.qn.b105}}</el-button>
            </h3>
                ...
                <template v-if="item.type != 3">
                    <table v-show="false"  :data-download-options="JSON.stringify({
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>{{$store.state.lang.qn.b104}}</td>
                                <td>{{$store.state.lang.qn.b10}}</td>
                            </tr>
                            <tr v-for="row in item.tableData" :key="row.id">
                                <td>{{row.name}}</td>
                                <td>{{row.count}}</td>
                    ...
                    <div :ref="item.id">
                        <el-table v-show="item.tab == 'table'" :data="item.tableData" border style="width: 100%">
                            <el-table-column prop="name" :label="$store.state.lang.qn.opt" 
                                align="center" width="">
                            ...
                            <el-table-column prop="count" v-if="item.type != 2" :label="$store.state.lang.qn.b104" align="center" width="180"></el-table-column>
...
export default {
    ...
    methods: {
        ...
        getData(){
            ...
            request('get', '/api/qn/qn_vote_statistics/title', { qnId: this.$route.query.id }).then(res=>{
                this.makeDate(res);
